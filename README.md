...
- **三层检索漏斗**:
  1. **Redis 缓存** (毫秒级, 命中率 ~31.5%) — 精确匹配 → 直接返回
  2. **BM25 FAQ** (~10ms, 命中率 ~22.8%) — jieba分词 + BM25Okapi + softmax门控
  3. **RAG 语义检索** (~2s, 覆盖率 ~46%) — 混合检索 + BGE精排 + LLM生成
...
- **写操作**: 改收货地址、取消订单、申请退款（含二次确认机制）还没完善
